// Based on bgrins/nativesortable

// Simplified UMD: http://webreflection.blogspot.com/2013/01/a-simplified-universal-module-definition.html
(this.define || Object)(
this.EvergreenSortable = function() {
  ...
  function init(element, options) {
    ...
    var stop = options.stop || function() { };
    ...
    var handleDragEnter = delegate(function(e) {
      ...
      if (element != currentlyDraggingElement.parentNode) {
        if(!options.connectWith) return true;
        if(element.className.indexOf(options.connectWith.substring(1)) === -1) {
          console.log(element.className,options.connectWith);
          return true;
        }
        // todo: check for presence of the DOMToken instead of checking indexOf
        if(options.connectWith.substring(0,1) !== '.' && element.tagName !== options.connectWith) {
          return true;
        }
